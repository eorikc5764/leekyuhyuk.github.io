---
layout  : post
title   : "[Build Own Linux] Root File System"
date    : 2020-01-26 23:41:12 +0900
category: Raspberry-Pi
---

[이전 글]({{ site.url }}/article/raspberry-pi/2020/01/26/Build-Own-Linux-Toolchain)에서 우리는 Toolchain을 빌드 하였습니다. 이번 시간에는 Root File System을 빌드 해보겠습니다.
...
빌드 과정은 크게 9개의 단계로 나누어져 있습니다:
  ...
  - **Step 1 :** 루트 파일 시스템의 기본적인 디렉터리를 생성합니다.
  - **Step 2 :** 필수적인 파일(`passwd`, `shadow`, `group`)을 생성합니다.
    - `passwd` : 사용자 로그인 계정, UID, GID, 이름, 홈 디렉토리, 로그인 쉘이 저장되어 있습니다.
    - `shadow` : 사용자 계정에 대한 암호화된 사용자 암호를 저장하고 있습니다.
    - `group` : 그룹 목록과 그룹에 속한 사용자 목록이 저장되어 있습니다.
  - **Step 3 :** GCC 라이브러리를 루트 파일 시스템으로 복사합니다.
  - **Step 4 :** Glibc를 빌드하고, 루트 파일 시스템에 설치합니다.
  - **Step 5 :** Busybox를 빌드하고, 루트 파일 시스템에 설치합니다.
    - Busybox : 리눅스 상에서 자주 사용되는 명령어들을 모은 프로그램입니다.
    - 사용 가능한 명령어는 '[Busybox 사이트](https://www.busybox.net/downloads/BusyBox.html)'의 'COMMANDS'를 참고하세요.
  - **Step 6 :** Bootscript를 루트 파일 시스템에 설치합니다.
  - **Step 7 :** 기본적인 네트워크 설정을 합니다.
  - **Step 8 :** Shell 설정을 합니다.
  - **Step 9 :** `fstab` 파일을 생성합니다.
    - `fstab` : 파일시스템 정보를 저장하고 있으며, 리눅스 부팅 시 마운트 정보를 저장하고 있습니다.
